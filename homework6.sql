...
SELECT filename 
FROM media 
WHERE size <=100000 AND media_type_id IN (SELECT id FROM media_types WHERE id=1)
...
SELECT firstname 
FROM users 
WHERE id IN (SELECT p.user_id, MIN(p.birthday) FROM profiles p)

/*10. Выведите количество женщин из каждого города*/

SELECT DISTINCT p.hometown, count(p.gender) 
AS 'Womens' 
FROM profiles p
WHERE p.gender = 'f' 
GROUP BY p.hometown